...
// yaw (z-axis rotation)
double quaternionToYawAngle(geometry_msgs::Quaternion q)
{
    double angle;
...
geometry_msgs::Quaternion rpyToQuaternion(const double roll, const double pitch, const double yaw)
{
...
geometry_msgs::Quaternion poseFrom2DVelocity(const double vx, const double vy)
{
  ...
  return rpyToQuaternion(0.0, 0.0, theta);
}

std::vector<std::pair<float, float>> LineObstacleToCells(const float x1, const float y1, const float x2, const float y2)
{
  int i;             // loop counter
